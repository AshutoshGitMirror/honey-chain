...
- Render demo 2026-08-28: user requested hosting via MCP. Plan: private GitHub repo `honey-chain`, push `app.py`, `schema.sql`, `README.md`, `requirements.txt`, Render web service using `python` runtime, `python app.py` with `$PORT` support. Keep SQLite on disk for demo, note ephemeral. Verify with `list_services` and `get_service`.
